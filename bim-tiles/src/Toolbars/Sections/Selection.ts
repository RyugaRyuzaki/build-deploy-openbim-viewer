...
export default (components: OBC.Components, _world?: OBC.World) => {
  ...
  //   if (ifcDetection.isTraining) {
  //     ifcDetection.isTraining = false;
  //     ifcDetection.visibleTimeCount = false;
  //     return;
  //   }
  //   if (!world) return;
  ...
  //   ifcDetection.isTraining = true;
  //   ifcDetection.visibleTimeCount = true;
  // };
  // const trainButton = BUI.Component.create<BUI.Button>(() => {
  //   return BUI.html`
  //   <bim-button @click=${onFocusSelection} label="Train" icon="ri:focus-mode" tooltip-title="Train" tooltip-text="Focus the camera to the current selection."></bim-button>
  //   `;
  // });
  // customIfcStreamer.onFragmentsLoaded.add((frags) => {
  //   const selection = ifcDetection.selectionSignal.value;
  //   for (const frag of frags) {
  //     if (selection) {
  //       const item = selection[frag.id];
  //       if (!item) frag.setVisibility(false);
  //     }
  //   }
  // });
